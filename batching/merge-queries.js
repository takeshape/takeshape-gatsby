...
const Visitors = {
  ...
  prefixVariables: prefix => {
    return {
      [_graphql.Kind.VARIABLE]: variable => prefixNodeName(variable, prefix)
    };
  },
  prefixFragmentNames: (prefix, fragmentNames) => {
    return {
      [_graphql.Kind.FRAGMENT_DEFINITION]: def => fragmentNames.has(def.name.value) ? prefixNodeName(def, prefix) : def,
      [_graphql.Kind.FRAGMENT_SPREAD]: def => fragmentNames.has(def.name.value) ? prefixNodeName(def, prefix) : def
    };
  }
};
...
function prefixQueryParts(prefix, query) {
  ...
  document = (0, _graphql.visit)(document, (0, _graphql.visitInParallel)([// Note: the sequence is important due to how visitInParallel deals with node edits
  ...
  if (fragmentsWithVariables.size > 0) {
    ...
    const visitorKeyMap = {
      [_graphql.Kind.DOCUMENT]: ['definitions'],
      [_graphql.Kind.OPERATION_DEFINITION]: ['selectionSet'],
      [_graphql.Kind.FRAGMENT_DEFINITION]: ['selectionSet'],
      [_graphql.Kind.INLINE_FRAGMENT]: ['selectionSet'],
      [_graphql.Kind.FIELD]: ['selectionSet'],
      [_graphql.Kind.SELECTION_SET]: ['selections']
    };
    document = (0, _graphql.visit)(document, Visitors.prefixFragmentNames(prefix, fragmentsWithVariables), visitorKeyMap);
  }
...
function aliasTopLevelFields(prefix, doc) {
  const transformer = {
    [_graphql.Kind.OPERATION_DEFINITION]: def => {
      const {
        selections
      } = def.selectionSet;
      return { ...def,
  ...
  return (0, _graphql.visit)(doc, transformer, keyMap);
}
/**
...
function aliasFieldsInSelection(prefix, selections, document) {
  return _lodash.default.flatMap(selections, selection => {
...
function inlineFragmentSpread(spread, document) {
  const fragment = document.definitions.find(def => def.kind === _graphql.Kind.FRAGMENT_DEFINITION && def.name.value === spread.name.value);
...
function isQueryDefinition(def) {
  return def.kind === _graphql.Kind.OPERATION_DEFINITION && def.operation === `query`;
}

function isFragmentDefinition(def) {
  return def.kind === _graphql.Kind.FRAGMENT_DEFINITION;
}